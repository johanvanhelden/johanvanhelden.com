...
if [ -f "$HOME/configfiles/.php_cs" ]; then
    ...
    echo '============================='
    php-cs-fixer fix app database config routes tests --dry-run --diff --allow-risky=yes --config=$HOME/configfiles/.php_cs
    if [ $? != 0 ]; then
        exit 1;
    fi
fi
...
done <<<$(grep -r -B 1 --include=*.php ".*function.*)$" app database tests)
...
echo '============================='
phpcpd app database config routes tests
if [ $? != 0 ]; then
    exit 1;
fi
echo
